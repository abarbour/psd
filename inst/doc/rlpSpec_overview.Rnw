...
\begin{abstract}
  ...
  Assuming the procedure is convergent, this method
  will produce
  spectra with significantly
  reduced variance (compared to \naive{} estimators),
  and minimum biasing effects.
  Resolution and uncertainty of any multitaper-based
  spectrum estimation
  are controlled by the number of tapers applied, which
  means we do
  ...
  \rlp{} is best suited for data having a mix of
   large dynamic range and narrow-band spectral features, such
   as power spectra of ambient seismic noise.
\end{abstract}
...
sampled once every kilometer (km), which means the spectrum may represent
...
We expect the MAGSAT data to follow an linear trend in the space of
linear-frequencies and logarithmic-power; hence,
we see a clear
...
PSD for the raw series looks somewhat unrealistic at higher wavelengths, and
shows obvious curvature bias.
%
\begin{figure}[htbp!]
\begin{center}
...
As a matter of bookkeeping and good practive, 
we should deal with the working environment
accessed by \rlp{} functions.
To ensure
\Rcmd{psdcore} does not access any inappropriate information we
...
the \Rcmd{psdcore} results, because they both produced uniformly tapered spectra
with the same number of sine tapers.
<<eval=TRUE, echo=TRUE>>=
...
@
and show the regression and residuals in Figure \ref{fig:psdreg}.  
...
\begin{figure}[htbp!]
\begin{center}
<<eval=TRUE, echo=FALSE, fig=TRUE, width=7, height=6, label=HIST1>>=
seqcols <- 1:numit
itseq <- seqcols - 1
toadd <- matrix(rep(itseq, numd), ncol=numit, byrow=T)
par(xpd=TRUE)
matplot(Freqs, StgPsd + (sc<-7)*toadd, type="l", lty=1, lwd=2, col="black",
...
@
<<eval=TRUE, echo=FALSE, fig=TRUE, width=7, height=4.5, label=HIST2>>=
...
invisible(lapply(rev(seqcols), FUN=function(mcol, niter=numit, Frq=Freqs, Dat=StgTap, cols=Cols){
  ...
  y <- c(595,595,650,650,595)
  text(mean(x),min(y)-.5*diff(range(y)), mcol-1)
