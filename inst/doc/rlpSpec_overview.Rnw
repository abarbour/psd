...
A proxy for this is simply $1/\sqrt{K}$, where $K$ is the number of
tapers.  Using $\nu = 2*K$ we can estimate the distribution
of uncertainties.  
Among other calculations, \Rcmd{spectral\_properties} returns the 
$\chi{}_{\nu}^2$ based uncertainties, as well as the
approximate uncertainties.  To illustrate, we 
...
based on $1/\sqrt{K}$, which is accurate to within a few percent.
}
\label{fig:psderr}
\end{center}
\end{figure}
...
lwds <- c(1,rep(2,numit-1))
matplot(Freqs, StgPsd, type="l", lty=1, lwd=lwds, 
             main="PSD estimation history")
@
<<eval=TRUE, echo=FALSE, fig=TRUE, width=7, height=4, label=HIST2>>=
matplot(Freqs, StgTap, type="l", lty=1, lwd=lwds, 
             main="Taper optimization history")
@
