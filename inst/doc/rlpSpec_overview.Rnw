...
\Rcmd{niter}
iterations of refinement (defaulting to 3).
With each iteration
...
Aspec <- pspectrum(ats, dt, tapinit, niter=2)
...
the
\Rcmd{psdcore} results, since they both produced uniformly tapered
spectra.
<<eval=TRUE, echo=TRUE>>=
...
In a multitaper algorithm the uncertainty and resolution vary with the
number of tapers in which the number of tapers controls the
effective degrees of freedom $\nu$ of a $\chi{}_\nu^2$ distribution.
This goes approximately as
$$\sqrt(10*K/12)^{-1}$$
and can be calculated with \Rcmd{spectral\_properties}.  For example:
<<eval=TRUE, echo=TRUE>>=
...
% instead plot adaptive history! [ ]
<<eval=TRUE, echo=TRUE, fig=TRUE, width=7, height=6, label=UNCERT>>=
plot(Pspec$freq, 100*spp$stderr, type="h", col="grey", ylim=c(0,30))
lines(Pspec$freq, 100*spp$stderr)
lines(Aspec$freq, 100*spa$stderr, type="h", col="light grey")
lines(Aspec$freq, 100*spa$stderr, col="red", lwd=2)
@

\begin{figure}[htbp!]
\begin{center}
<<eval=TRUE, echo=TRUE, fig=TRUE, width=7, height=6, label=POLY>>=
...
So uncertainty is reduced as the number of tapers is increased.
Even though a multitaper spectral estimate can have much lower uncertainty
than a \naive{} estimator, the amount of bias introduced by
tapers may not be trivial.  The approach of \citet{rs1995} is
to find the number of tapers which minimizes any potential bias.

% plot resolution: the opposite should be true [ ]
