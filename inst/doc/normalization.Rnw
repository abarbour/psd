...
$f$
having Fourier Transform $\FTp{f}$ represented by
$F$.
The \index{amplitude spectrum} of this transform pair is simply the 
amplitude of complex $F$, or $\mathrm{mod} \{F\}$; we will denote 
this as $^{\mathtt{(A)}} S$.
The corresponding
