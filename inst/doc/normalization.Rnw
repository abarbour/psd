...
of length $N$,
having Fourier transform \dFTp{f} which is complex, and represented by\dF{}.
...
We first generate a series, and find its Fourier transform\footnote{
...
@
followed by the energy spectral densities\footnote{Note the equivalence
...
The single-sided power spectral density (PSD) estimates
follow, once we
set the Nyquist frequency, defined as half the sampling rate\footnote{
Although the PSD of a white noise process is not strictly bandlimited,
...
The dashed rectangle encompasses the area under the mean spectral level,
used to find the integrated spectrum.}
\end{center}
\end{figure}

Figure \ref{fig:psdN} plots the PSD of our white noise series; it also shows
the mean value of the PSD, from which we can test ourselves for
...
To compare the scalings it will be useful to show the spectral values
in decibels (relative to units$^2/$frequency):
