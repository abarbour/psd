...
{\ttfamily\noindent\itshape\color{messagecolor}{\#\# Loaded psd (0.2.0) -- Adaptive multitaper spectrum estimation.}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\itshape\color{messagecolor}{\#\# Normalized\ \ single-sided PSD\ \ (PSD)\ \ to sampling-freq.\ \ 1}}\begin{alltt}
...
{\ttfamily\noindent\itshape\color{messagecolor}{\#\# Normalized\ \ single-sided PSD\ \ (PSD)\ \ to sampling-freq.\ \ 1}}\end{kframe}
\end{knitrout}
...
An intriguing method for producing power spectral density estimates
 ...
 is presented by \citet{rover2011}.
 The method uses the \emph{Student's t} distribution
 to estimate the distribution of spectral densities
 at a given frequency and is packaged into
 \Rcmd{bspec} as the function \Rcmd{bspec}.
...
PSDs with and without adaptive taper optimizaiton, respectively.
...
\begin{alltt}
frq <- Aspec$freq
relp <- (\hlfunctioncall{sqrt}(\hlfunctioncall{varddiff}(Aspec$spec))/spa$resolution - 0)
relp <- \hlfunctioncall{dB}(spa$stderr.chi.upper/spa$resolution - 0)
relp <- ((spa$resolution) - spp$resolution - 0)/spp$resolution
\hlfunctioncall{par}(las = 1, oma = \hlfunctioncall{rep}(0, 4), omi = \hlfunctioncall{rep}(0, 4))
\hlfunctioncall{par}(mar = \hlfunctioncall{c}(3, 3, 2, 0))
\hlfunctioncall{layout}(\hlfunctioncall{matrix}(\hlfunctioncall{c}(1, 2), 1, 2, byrow = TRUE), heights = \hlfunctioncall{c}(2, 2), widths = \hlfunctioncall{c}(3, 
    0.5), respect = TRUE)
\hlfunctioncall{plot}(frq, relp, col = \hlstring{"light grey"}, ylim = yl <- \hlfunctioncall{c}(0, 50), type = \hlstring{"h"}, xaxs = \hlstring{"i"}, 
    yaxs = \hlstring{"i"}, ylab = \hlstring{"dB"}, xlab = \hlstring{"frequency, 1/km"}, main = \hlstring{"Percent increase in spectral resolution"})
\hlfunctioncall{lines}(frq, relp)
\hlfunctioncall{par}(mar = \hlfunctioncall{c}(3, 0, 2, 2))
\hlfunctioncall{boxplot}(relp, axes = FALSE, ylim = yl, yaxs = \hlstring{"i"}, notch = TRUE, main = \hlfunctioncall{sprintf}(\hlstring{"%.01f"}, 
    \hlfunctioncall{median}(relp)))
...
Resolution limits for Project MAGNET PSD (\Rcmd{niter=5} adapts--the default) 
...
##  [1] bspec_1.4          ggplot2_0.9.3      signal_0.7-3      
##  [4] MASS_7.3-23        RColorBrewer_1.0-5 RSEIS_3.0-9       
##  [7] Rwave_2.0          RPMG_2.1-4         psd_0.2-0         
...
##  [9] labeling_0.1     lattice_0.20-13  munsell_0.4      plyr_1.8        
## [13] proto_0.3-10     reshape2_1.2.2   scales_0.2.3     stringr_0.6.2   
## [17] tools_2.15.2     zoo_1.7-9
\end{verbatim}
\end{kframe}
\end{knitrout}
