...
plot(psdc, log="dB", main="Raw and cleaned Project MAGNET power spectral density estimates", 
...
mtext("(a)", font=2, adj=0, line=-0.8)
mtext("PSDs by stage", line=-0.8, font=4)
@
...
invisible(lapply(rev(seqcols), FUN=function(mcol, niter=numit, Frq=Freqs, Dat=StgTap, cols=Cols){
  ...
  y <- c(0.95, 0.95, 1, 1, 0.95) * 0.95 * ylm
  text(mean(x), ylm*0.95**2, mcol-1, cex=0.9, pos=1, font=2)
...
text(0.25, -14.5, "(pilot spectrum -- uniform tapers)", font=3, cex=0.8)
@
