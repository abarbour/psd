...
library(knitr)
options(width=69)
knitr::opts_chunk$set(tidy = TRUE, size="small", split=TRUE)
par(las=1)
@
...
@
<<eval=TRUE, echo=FALSE, fig.width=6.0, fig.height=4.0, label=Outliers>>=
plot(raw ~ km, magnet, col=NA, las=1, ylab="Magnetic anomaly", xlab="Along-track distance, km")
...
\begin{figure}[!htbp]
\begin{center}
<<eval=TRUE, echo=FALSE, fig.width=7, fig.height=7.5, label=RAWvCLEAN>>=
...
a <- rnorm(32)
all.equal(psdcore(a,1,1,first.last=FALSE)$spec, psdcore(a,-1,1,first.last=FALSE)$spec)
@
...
\begin{figure}[!htbp]
\begin{center}
<<eval=TRUE, echo=TRUE, fig.width=6, fig.height=3.0, label=MAGRES>>=
frq <- Aspec$freq
...
\begin{figure}[!htbp]
\begin{center}
<<eval=TRUE, echo=FALSE, fig.width=4.8, fig.height=2.4, label=HIST1>>=
seqcols <- seq_len(numit)
itseq <- seqcols - 1
toadd <- matrix(rep(itseq, numd), ncol=numit, byrow=TRUE)
PltStg <- StgPsd + (sc<-6)*toadd
par(xpd=TRUE, oma=rep(0,4), mar=c(1,4,3,2), tcl=-0.4)
...
mtext("(a)", font=2, adj=0, line=-0.3)
mtext("PSDs by stage", line=-0.3, font=4)
@
<<eval=TRUE, echo=FALSE, fig.width=4.8, fig.height=2.4, label=HIST2>>=
par(xpd=TRUE, las=1, oma=rep(0,4), mar=c(1,4,2,2), tcl=-0.4)
