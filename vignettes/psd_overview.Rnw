...
require(bspec)
print(Bspec <- bspec(magnet$clean))
@
...
\begin{figure}[htb!]
\begin{center}
<<eval=TRUE, echo=TRUE, fig.width=7, fig.height=4.5, label=SPECERR>>=
...
par(las=1, oma=rep(0,4), omi=rep(0,4))
par(mar=c(3,3,2,0))
...
plot(frq, relp,
     ...
     main="Percent increase in spectral resolution")
lines(frq, relp)
par(mar=c(3,0,2,2))
boxplot(relp, axes=FALSE, ylim=yl, yaxs="i", notch=TRUE, main=sprintf("%.01f",median(relp)))
axis(4)
@
...
\begin{figure}[htb!]
\begin{center}
<<eval=TRUE, echo=FALSE, fig.width=6.0, fig.height=4.0, label=HIST1>>=
seqcols <- seq_len(numit)
itseq <- seqcols - 1
toadd <- matrix(rep(itseq, numd), ncol=numit, byrow=T)
par(xpd=TRUE, oma=rep(0,4), mar=c(1,4,2,2))
matplot(Freqs, StgPsd + (sc<-9)*toadd, type="l", lty=1, lwd=2, col="black",
             main="PSD estimation history", ylab="", xlab="",
             yaxt="n", frame.plot=FALSE)
text(0.52, 1.05*sc*itseq, itseq)
lines(c(2,1,1,2)*0.01,c(0,0,7,7))
text(.06, 3.5, "7 dB")
mtext("Spectrum optimization stages", line=-0.8)
@
<<eval=TRUE, echo=FALSE, fig.width=6.0, fig.height=3.0, label=HIST2>>=
par(xpd=TRUE, las=1, oma=rep(0,4), mar=c(4,4,2,2))
...
invisible(lapply(rev(seqcols), FUN=function(mcol, niter=numit, Frq=Freqs, Dat=StgTap, cols=Cols){
  ...
  if (iter==1){
    plot(Frq, y, type="h", col=icol, 
           main="", ylab="", xlab="Spatial frequency",
           ylim=c(0,550), yaxs="i", frame.plot=FALSE)
  } else {
    lines(Frq, y, type="h", col=icol)
  }
  lines(Frq, y, type="l",  lwd=0.9)
  x <- (c(0,1)+iter-1)*.05+0.075
  y <- c(545,545,600,600,545)
  text(mean(x),max(y)-1.7*diff(range(y)), mcol-1)
  polygon(c(x,rev(x),x[1]),y,border="black",col=icol)
}))
mtext("Taper optimization stages", line=1)
@
