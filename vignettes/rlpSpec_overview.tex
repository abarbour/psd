...
%#psdc_ar <- pspectrum(magsat$clean, AR=TRUE, plot=FALSE)
%@
%
\begin{figure}[htb!]
\begin{center}
...
Figure \ref{fig:magd} plots the innovations from the AR
...
\hlfunctioncall{plot}(\hlfunctioncall{ts.union}(orig.plus.trend = mc, linear = ats_lm, ar = ats_ar), yax.flip = TRUE, 
    main = \hlfunctioncall{sprintf}(\hlstring{"Prewhitened MAGSAT series: linear and linear+\hlfunctioncall{AR}(%s)"}, atsar$ardfit$order))
\end{alltt}
\end{kframe}
...
%<<eval=TRUE, echo=TRUE, fig.height=2.75, fig.width=4.5, label=ARPSD>>=
%plot(psdcore(atsar$prew_ar, ntaper=10), log="dB", main="PSD of MAGSAT innovations")
%@
\caption{Pre-whitening of the MAGSAT series (with a
synthetic trend superimposed on it) assuming linear and linear-with-AR models.
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
psdc_ar <- \hlfunctioncall{pspectrum}(magsat$clean, AR = TRUE, niter = 0, plot = TRUE)
\end{alltt}
...
%@
%\caption{Power spectral density estimates for the raw and cleaned
%   \Rcmd{MAGSAT}, with and without auto-regressive (AR) prewhitening.}
%\label{fig:arpsd}
%\end{center}
%\end{figure}
...
\caption{Resolution limits for MAGSAT PSD (\Rcmd{niter=5} adapts--the default) 
