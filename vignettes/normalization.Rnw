...
\subsection{Power spectral density: An informal definition}
...
\begin{figure}[htb!]
\begin{center}
<<eval=TRUE, echo=TRUE, fig.width=6, fig.height=3, label=FILTERS>>=
...
\subsection{Power spectral density: A more formal definition}
Let us return to having a single realization
of a stochastic process in the time domain, 
\dXstoch{}, which is sampled over a finite interval of time
...
We can use a property of the ACV function \dACV{\tau} or
...
sampled at 1 Hz, 
will be flat at \unitfrac[2]{units$^2$}{Hz}
