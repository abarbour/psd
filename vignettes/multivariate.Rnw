...
library(psd)
data(wipp30)
wipp30 <- as.matrix(wipp30[, c(3,4,2)]) # output as last column, input as first two
freqs <- 1:(nrow(wipp30)/2) * (24) / nrow(wipp30) # frequency in cpd
@

<<eval=TRUE, echo=TRUE, label="calcmv">>=
mv <- pspectrum(wipp30, riedsid_column= 0L, verbose = FALSE, plot = FALSE)
@
...
pspectrum(wipp30, riedsid_column= 0L, verbose = FALSE, plot = TRUE)
@
...
mn <- pspectrum(wipp30, riedsid_column= 0L, verbose = FALSE)   # minimum number 
mx <- pspectrum(wipp30, riedsid_column=-1L, verbose = FALSE)   # maximum number
c1 <- pspectrum(wipp30, riedsid_column= 1L, verbose = FALSE)   # column 1
c2 <- pspectrum(wipp30, riedsid_column= 2L, verbose = FALSE)   # column 2
c3 <- pspectrum(wipp30, riedsid_column= 3L, verbose = FALSE)   # column 3
...
one_val <- pspectrum(wipp30, riedsid_column= 0L, verbose = FALSE,
...
par(mar = c(3, 4, 1, 0), mgp = c(2, 0.5, 0))
vec_val <- pspectrum(wipp30, riedsid_column= 0L, verbose = FALSE,
                 niter = 0, ntap = as.tapers(1:(nrow(wipp30) %/% 2) %/% 7 + 7), plot = TRUE)
plot(vec_val$taper)
@
...
spec <- pspectrum(wipp30, riedsid_column= 0L, verbose = FALSE)$pspec
dim(spec)
...
coh <- pspectrum(wipp30, riedsid_column= 0L, verbose = FALSE,
                 niter = 0, ntap.init = 11)$coh
coh_base <- spec.pgram(wipp30[, c(3,1,2)], spans = c(11), fast = FALSE, plot = FALSE)$coh
@
...
phase <- pspectrum(wipp30, riedsid_column= 0L, verbose = FALSE,
                 niter = 0, ntap.init = 11)$phase
phase_base <- spec.pgram(wipp30[, c(3,1,2)], spans = c(11), fast = FALSE, plot = FALSE)$phase
@
...
transfer <- pspectrum(wipp30, riedsid_column= -1L, verbose = FALSE)$transfer
gain <- Mod(transfer)
@
...
par(mar = c(3, 3, 2, 0), mgp = c(2, 1, 0))
plot(gain[, 1]~freqs, type='l', xlim = rng,
     ylab = 'WL and Barometric',
     xlab = 'Frequency (cpd)',
     main = 'Gain', ylim = c(0, 3))
plot(gain[, 2]~freqs, type='l', xlim = rng, ylim = c(0, 0.0001),
     ylab = 'WL and Earth tide',
     xlab = 'Frequency (cpd)',
     main = 'Gain')
@
...
plot(phase[, 1]~freqs, type='l', xlim = rng,
     ylab = 'WL and Barometric',
     xlab = 'Frequency (cpd)',
     main = 'Phase')
plot(phase[, 2]~freqs, type='l', xlim = rng, 
     ylab = 'WL and Earth tide',
     xlab = 'Frequency (cpd)',
     main = 'Phase')
@
