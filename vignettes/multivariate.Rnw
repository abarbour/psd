...
library(psd)
data(wipp30)
wipp30 <- as.matrix(wipp30[, -1]) # remove the time column for this vignette
freqs <- 1:(nrow(wipp30)/2) * (24) / nrow(wipp30) # frequency in cpd
@

<<eval=TRUE, echo=TRUE, label="calcmv">>=
mv <- pspectrum(wipp30, riedsid_column= 0L, verbose = FALSE, plot = FALSE, output_column = 1)
@
...
coh_base <- spec.pgram(wipp30, spans = c(11), fast = FALSE, plot = FALSE)$coh
@
...
phase_base <- spec.pgram(wipp30, spans = c(11), fast = FALSE, plot = FALSE)$phase
@
