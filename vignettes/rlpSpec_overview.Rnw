...
\begin{center}
%% magsat compare
<<eval=TRUE, echo=TRUE,  label=MAGSATNAIVE>>=
ntap <- psdc$taper
...
require(RSEIS)
dt=1 # km
summary(prewhiten(mc <- ts(magsat$clean, deltat=dt)+seq_along(magsat$clean), plot=FALSE))
...
plot(ts.union(orig.plus.trend=mc, linear=ats_lm, ar=ats_ar), yax.flip=TRUE, 
     main=sprintf("Prewhitened MAGSAT series: linear and linear+AR(%s)", atsar$ardfit$order))
@
...
<<eval=TRUE, echo=TRUE, label=ARFIT>>=
summary(atsar)
...
<<eval=TRUE, echo=TRUE, label=MAGPSDAR>>=
psdc_ar <- pspectrum(magsat$clean, AR=TRUE, niter=0, plot=TRUE)
@
