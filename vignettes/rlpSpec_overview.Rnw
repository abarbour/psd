...
psdr <- pspectrum(magsat$raw, plot=FALSE)
psdc <- pspectrum(magsat$clean, plot=FALSE)
@
...
\begin{figure}[htb!]
\begin{center}
<<eval=TRUE, echo=TRUE, fig.height=3.8, fig.width=6.5, label=ARFIT>>=
atsar <- prewhiten(ats, AR.max=100, verbose=FALSE)$prew_ar
@
