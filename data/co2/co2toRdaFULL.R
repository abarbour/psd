...
(p + 
  ...
  scale_y_continuous("Power, dB rel ppm-CO2**2 * year", limits=c(-100,22), expand=c(0,0))+
  scale_colour_manual("Spectrum\nestimator", values=pal[1:2])+
  scale_size_manual(values=c(0.4,0.8), legend=F)
 #+
  #opts(legend.position=c(1.0,0.5))
)
